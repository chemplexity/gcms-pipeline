...
peaksFields = {
    ...
    'xmin', 'xmax', 'ymin', 'ymax', 'model', 'revision', ...
    ...
    'match_compound_retention_time', 'match_compound_exact_mass', 'match_db_id', 'match_comments', ...
    'match_num_peaks', 'match_mz', 'match_intensity'};
...
for i = 1:length(data)
    ...
    for j = 1:length(data(i).peaks)
        ...
        % Add sample data
        peaksData(end+1,1).sample_index = i;
        peaksData(end,1).peak_index = j;
        peaksData(end,1).file_path = data(i).file_path;
        ...
        if isfield(data, 'checksum')
            peaksData(end,1).file_checksum = data(i).checksum;
        end
